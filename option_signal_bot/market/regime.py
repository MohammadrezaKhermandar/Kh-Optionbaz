...
* **تعدیلِ قیمت:** برای **سهم**، قیمتِ تاریخی باید با افزایش سرمایه
  تعدیل شده باشد وگرنه یک ریزشِ ساختگی دیده می‌شود
  (`market/corporate_actions.py`). این ماژول خودش تعدیل نمی‌کند؛
  فراخواننده وضعیت را اعلام می‌کند و همان در گزارش می‌آید — «تعدیل شد»،
  «لازم نبود» یا «نامعلوم». شاخص تعدیل نمی‌خواهد.
"""
...
class Adjustment(str, Enum):
    """وضعیتِ تعدیلِ قیمتِ تاریخی — سه حالت، چون دو تای آخر یکی نیستند."""
...
ADJUSTMENT_LABELS: dict[Adjustment, str] = {
    Adjustment.APPLIED: "قیمت‌ها با رویدادهای شرکتی تعدیل شده‌اند",
    Adjustment.NOT_NEEDED: "تعدیل موضوعیت ندارد (شاخص)",
    Adjustment.UNKNOWN: (
        "وضعیت تعدیل نامعلوم است؛ اگر افزایش سرمایه‌ای در این بازه بوده "
        "باشد، تغییرِ قیمت می‌تواند ساختگی باشد"
    ),
}
...
def assess_regime(
    ...
    *,
    ...
    if adjustment is Adjustment.UNKNOWN and subject_kind == "underlying":
        reasons.append(ADJUSTMENT_LABELS[Adjustment.UNKNOWN] + ".")
