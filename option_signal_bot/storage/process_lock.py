...
**مسئله:** دو نویسنده‌ی recorder روی یک پایگاه یعنی دو دریافت هم‌زمان
و دو snapshot موازی از یک لحظه — چه دو حلقه باشند، چه یک حلقه و یک
اجرای تکی، چه دو اجرای تکی. `asyncio.Lock` داشبورد
    ...
    def __enter__(self) -> ProcessLock:
        ...
        if not _acquire(handle):
            ...
            raise LockUnavailable(
                ...
                f"اجراست. دو نویسنده روی یک پایگاه، snapshotهای موازی از "
                f"یک لحظه می‌سازند؛ اول آن یکی را متوقف کنید."
            )
        self._handle = handle
        return self
