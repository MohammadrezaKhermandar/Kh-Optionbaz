...
def default_settings() -> dict[str, Any]:
    ...
    return {
        ...
        # تحلیل وضعیت بازار و نماد پایه — **جدا از امتیاز**.
        #
        # ⚠️ آستانه‌ها فرضِ اولیه‌اند، نه نتیجه‌ی پژوهش. این تشخیص
        # توصیفِ وضعیتِ فعلی است، نه پیش‌بینی تا سررسید، و هیچ درصد
        # اطمینانی نمی‌سازد.
        "regime": {
            "enabled": True,
            # افق تحلیل و کفِ دادهٔ لازم، به تعداد جلسه‌ی معاملاتی
            "horizon_sessions": 20,
            "min_sessions": 40,
            # آخرین جلسه از این کهنه‌تر باشد (روز تقویمی) ⇒ نامشخص
            "max_staleness_days": 7,
            # کاراییِ حرکت (۰ تا ۱) و نسبتِ تغییر به نوسانِ همان بازه
            "min_efficiency": 0.30,
            "min_normalized_move": 0.50,
            # چند جلسه تاریخچه خوانده شود (برای نوسان، بیشتر از افق)
            "history_days": 200,
            # «بازار» = شاخص کل بورس تهران
            "market_ins_code": "32097828799138957",
            "market_label": "شاخص کل بورس تهران",
            # برای تست/آفلاین: پوشه‌ی پاسخِ ضبط‌شده‌ی شاخص
            "index_history_dir": None,
            # پرسیدنِ رویدادهای شرکتی یک درخواست شبکه است. خاموش که
            # باشد، وضعیتِ تعدیل «نامعلوم» گزارش می‌شود — نه «نبوده».
